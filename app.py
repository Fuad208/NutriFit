...
def find_training_tutorial(exercise: dict, tutorials: list[dict]) -> dict | None:
    ...
    for tutorial in tutorials:
        ...
        token_overlap = len(title_tokens & name_tokens) / len(token_union) if token_union else 0
...
def workout_view(exercises: pd.DataFrame) -> None:
    ...
    if generate or st.session_state.exercise_recommendations is None:
        st.session_state.excluded_exercise_titles = []
    ...
    display_workouts(exercises, st.session_state.exercise_recommendations, load_training_tutorials(), workout_filters)
